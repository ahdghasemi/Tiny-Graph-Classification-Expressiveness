# -*- coding: utf-8 -*-
"""Copy of Chap9_v2_GCN_GIN_pruning_inference_time_model_size_weight_quant.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1cAg1NbhnTfvW5gsK1n08O44s0YeEq8uz
"""
...
#     # Report accuracy before pruning
#     test_loss, test_acc = test(gcn, test_loader)
#     print(f'GCN Before Pruning - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     print()

#     time_before_pruning = measure_inference_time(gcn, test_loader)

#     # Apply pruning
#     gcn_pruned = apply_pruning_to_gcn(gcn)

#     time_after_pruning = measure_inference_time(gcn_pruned, test_loader)

#     # Report accuracy immediately after pruning (without fine-tuning)
#     test_loss, test_acc = test(gcn_pruned, test_loader)
#     print(f'After Pruning GCN (without fine-tuning) - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     print()

#     # Fine-tune the pruned model
#     gcn_pruned = train(gcn_pruned, train_loader)

#     test_loss, test_acc = test(gcn_pruned, test_loader)
#     print(f'Pruned GCN - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     #------------- end prunning
...
#     # gcn = train(gcn, train_loader)
#     # test_loss, test_acc = test(gcn, test_loader)
#     # print(f'GCN Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     plot_graphs(gcn, dataset, "GCN")
...
#     print(f'GIN Before Pruning - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     print()

#     # Save and get size before pruning
#     size_before_pruning = save_model_and_get_size(gin, "gin_before_pruning.pt")

#     time_before_pruning = measure_inference_time(gin, test_loader)

#     # 2. Apply pruning
#     gin = apply_pruning_to_gin(gin, pruning_rate=0.2)

#     time_after_pruning = measure_inference_time(gin, test_loader)

#     # Save and get size after pruning
#     size_after_pruning = save_model_and_get_size(gin, "gin_after_pruning.pt")

#     print(f"Model size before pruning: {size_before_pruning:.2f} MB")
#     print(f"Model size after pruning: {size_after_pruning:.2f} MB")

#     # Report accuracy immediately after pruning (without fine-tuning)
#     test_loss, test_acc = test(gin, test_loader)
#     print(f'After Pruning GIN (without fine-tuning) - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     print()

#     # 3. Fine-tune the pruned model
#     gin = train(gin, train_loader)

#     # Report accuracy after fine-tuning the pruned model
#     test_loss, test_acc = test(gin, test_loader)
#     print(f'After Fine-tuning - Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')

#     # Train and test GIN model
#     # gin = GIN(dim_h=32)
#     # gin = train(gin, train_loader)
#     # test_loss, test_acc = test(gin, test_loader)
#     # print(f'GIN Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}%')
#     plot_graphs(gin, dataset, "GIN")
